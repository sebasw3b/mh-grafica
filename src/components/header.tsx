    ...
    <AnimatePresence>
      {isOpen && (
        <motion.nav
          ...
          transition={{ duration: 0.3 }}
        >
          <div className="flex flex-col justify-between gap-4 text-xl mt-12">
            {items.map((item, index) => (
              ...
              >
                <a
                  href={item.href}
                  onClick={onClose}
                  className="block w-full text-left px-2 tracking-normal font-bold hover:text-accent transition-colors duration-200 text-xl font-secondary"
                >
                  {item.label}
                </a>
              </motion.div>
